			...
			<a-row>
				...
				<a-col :xs="24" :sm="24" :md="18" :lg="18">
					...
					<a-row :gutter="16" v-if="formData.user_type != 'staff_members'">
						...
						<!-- ScorpioN Modified -->
						<a-col :xs="24" :sm="24" :md="12" :lg="12" v-if="pageType == 'Customers'">
							<a-form-item
								:label="$t('user.supplier')"
								name="supplier_id"
						...
						<a-col :xs="24" :sm="24" :md="pageType == 'Customers' ? 12 : 24" :lg="pageType == 'Customers' ? 12 : 24">
							<a-form-item
...
export default defineComponent({
	...
	setup(props, { emit }) {
		...
		onMounted(() => {
			...
			const usersPromise = axiosAdmin.get("suppliers");
			Promise.all([rolesPromise, usersPromise]).then(([rolesResponse, usersResponse]) => {
				roles.value = rolesResponse.data;
				suppliers.value = usersResponse.data;
			});
